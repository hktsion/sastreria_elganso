...
(function(TAILOR, DOC, SS, SUMMARY, $){
	...
	function inicializarApp(){
		...
		swal({
			title: "¡Recuerda ser coherente con los colores de tu traje!<br>",
			text: "No todos los colores quedan bien juntos... puedes dejarte asesorar por nuestros sastres en tu visita a tienda",
			icon: "success",
			button: "Lo haré",
		});

		// ¡Recuerda ser coherente con
		// los colores de tu traje!
		// No todos los colores quedan bien juntos... puedes dejarte asesorar por nuestros sastres en tu visita a tienda

	}

	//------------------------------------------------------------------> FUNCIONES DE INICIALIZACIÓN

	function initForroSVG(){
		let docframe = document.getElementById('tipoforro');
		let fillElements = (docframe.contentDocument).querySelectorAll('.color2');
	...
	TAILOR.mostrarIlustracion = function(datav_a, datav_b){
		...
		$('.'+datav_a+'.'+datav_b).css({'left': -10+'%', 'opacity': 1});
	}
	...
	function addRadioColorsListeners(that, val){
		...
		if($(that).hasClass('radiobox')){ 
			...
			//El color de la chaqueta y el pantalón debe ser el mismo
			/*FALTA POR IMPLEMENTAR QUE EL "SÉ CONCECUENTE CON LOS COLORES DE CHAQUETA Y PANTALÓN"*/
	...
	function addRadioListeners(){
		...
		$('input:radio').change(function(){
			...
			if($(this).is(':checked')){
				...
				if(data.data[0] == 'chaqueta'){
					...
					if (data.data[1] == 'forro' || data.data[1] == 'colorforro') {
						...
						if(data.data[2] == 'sinforro'){
							$("[data-bind='chaqueta-forro']").parents('.row-level-2').show();
							MOVEMENTS.cambiarElForro(url+'forro_sinforro.svg', initForroSVG);
						}
						if(data.data[2] == 'conforro'){
							$("[data-bind='chaqueta-forro']").parents('.row-level-2').show();
							MOVEMENTS.cambiarElForro(url+'forro_conforro.svg', initForroSVG);
						}
						if(data.data[2] == 'medioforro'){
							$("[data-bind='chaqueta-forro']").parents('.row-level-2').show();
							MOVEMENTS.cambiarElForro(url+'forro_medioforro.svg', initForroSVG);
						}
					}
				}


				//LLAMADAS DE PANTALÓN
				if(data.data[0] == 'pantalon'){
					//TODO something with pantalón'
				}
				...
				alert('Ha ocurrido un error inesperado, por favor, ponte en contacto con nosotros. Gracias');
			}
	...
	//Inicializa al aplicación :: unico método accesible
